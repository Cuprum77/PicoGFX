...
*/
graphics::graphics(color_t *frameBuffer, display_obj *display_ptr)
{
...
/**
 * @brief Fill the display with a color
 * @param color color to fill with as a raw value
*/
void graphics::fillRaw(color_t color)
{
	color = COLOR_INV(color);
	uint32_t totalPixels = this->display_ptr->getWidth() * this->display_ptr->getHeight();
