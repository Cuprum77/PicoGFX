 ...
 * @param width Width of the bitmap
 * @param height Height of the bitmap
 * @param center If the image should be centered on the screen or not
 * @param transparentColor Color to treat as transparent, pixels with this color will not be drawn
*/
void graphics::drawBitmap(const uint32_t *bitmap, uint32_t width, uint32_t height, 
    bool center, uint32_t transparentColor)
{
    ...
    point start = point(startX, startY);
    this->drawBitmap(bitmap, width, height, start, transparentColor);
}
    ...
    point start, uint32_t transparentColor)
{
    ...
    for (int y = startY + offsetY, by = offsetY; y < endY; ++y, ++by)
    {
        for (int x = startX + offsetX, bx = offsetX; x < endX; ++x, ++bx)
        {
            ...
            // If the color is the transparent color, skip drawing this pixel
            if (colorWord == transparentColor)
                continue;
